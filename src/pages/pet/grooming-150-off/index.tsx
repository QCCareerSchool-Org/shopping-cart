...
const Grooming150Off = ({ courses, currencyCode }: Props): ReactElement => (
  <>
    <Promo />
    <Form
...
const DynamicMessage = ({ courses, currencyCode }: DynamicMessageProps): ReactElement | null => {
  const discount = currencyCode === 'GBP' ? '£150' : '$150';
  if (courses.length === 0) {
    return null;
